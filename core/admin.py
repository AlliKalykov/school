...
class GroupAdmin(admin.ModelAdmin):
    ...
    def view_students(self, obj):
        return format_html(
            f'<a href="/admin/core/student/?group__id__exact={obj.id}">{obj.student_set.count()} студентов</a>'
        )

    def view_raspisanie(self, obj):
        return format_html(
            f'<a href="/admin/core/raspisanie/?group__id__exact={obj.id}">{obj.raspisanie_set.count()} пар</a>'
        )
